...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversInsertedValues() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            XCTAssertNil(insertionError, "Expected insertion to succeed")
            sut.retrieve { retrieveResult in
                switch retrieveResult {
                    ...
                    default:
                        XCTFail("Expected found result with the \(feed) and timestamp \(timestamp), got \(retrieveResult) instead")
                }
                exp.fulfill()
            }
        }
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    switch (firstResult, secondResult) {
                        ...
                        default:
                            XCTFail("Expected retrieving twice from non emtpy cache to deliver the same result with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
                    }
                    exp.fulfill()
                }
            }
